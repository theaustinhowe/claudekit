...
export const CLAUDE_ERRORS = {
  ...
  DISABLED:
    "Claude Code is disabled in settings. Enable it in the Settings page.",
} as const;
    ...
    if (
      ...
      for (const block of msg.message.content) {
        if (block.type === "text" && block.text) {
          return (
            detectSignal(block.text) ?? { type: "text", content: block.text }
          );
        }
...
export async function startClaudeRun(
  ...
  if (resumeWithMessage && job.claude_session_id) {
    ...
    await emitLog(
      jobId,
      "system",
      `Resuming Claude session with message: ${resumeWithMessage}`,
      logState,
    );
  } else {
    ...
    if (job.pending_injection) {
      ...
      await execute(
        conn,
        "UPDATE jobs SET pending_injection = NULL, updated_at = ? WHERE id = ?",
      ...
      await emitLog(
        jobId,
        "system",
        `Including pending message: ${job.pending_injection}`,
        logState,
      );
    }
    ...
    await emitLog(
      jobId,
      "system",
      `Starting new Claude Code run (session: ${sessionId})...`,
      logState,
    );
  }
  ...
  if (claudeProcess.pid) {
    ...
    await emitLog(
      jobId,
      "system",
      `Claude process started (PID: ${claudeProcess.pid})`,
      logState,
    );
  } else {
    ...
    await emitLog(
      jobId,
      "stderr",
      "Failed to spawn Claude process - no PID assigned",
      logState,
    );
  ...
  if (!claudeProcess.stdout || !claudeProcess.stderr) {
    console.error(
      `[claude-code] Process spawned but stdio streams unavailable`,
    );
    await emitLog(
      jobId,
      "stderr",
      "Claude process spawned but stdio streams are not available",
      logState,
    );
  }
    ...
    for (const line of lines) {
      ...
      if (phaseInfo) {
        ...
        execute(
          ...
          .then(() =>
            queryOne<DbJob>(conn, "SELECT * FROM jobs WHERE id = ?", [
              jobId,
            ]).then((updated) => {
              if (updated) {
        ...
        case "session":
          if (parsed.sessionId) {
            ...
            await execute(
              conn,
              "UPDATE jobs SET claude_session_id = ?, updated_at = ? WHERE id = ?",
              [parsed.sessionId, new Date().toISOString(), jobId],
            );
            await emitLog(
              jobId,
              "system",
        ...
        case "signal":
          if (parsed.signal === "PLAN") {
            await emitLog(
              jobId,
              "system",
              "Agent submitted implementation plan",
              logState,
            );
            ...
            await emitLog(
              jobId,
              "system",
              `Agent needs info: ${parsed.question}`,
              logState,
            );
      ...
      if (job.issue_number > 0) {
        try {
          ...
          await emitLog(
            jobId,
            "stderr",
            `Failed to post plan to GitHub: ${err.message}`,
            logState,
          );
        }
      }
  ...
  claudeProcess.on("error", async (error) => {
    console.error(
      `[claude-code] Process error for job ${jobId}: ${error.message}`,
    );
    clearTimeout(timeoutId);
    activeProcesses.delete(jobId);
 ...
 */
export async function stopClaudeRun(
  jobId: string,
  saveSession = false,
): Promise<boolean> {
  const proc = activeProcesses.get(jobId);
  ...
  if (saveSession && proc.sessionId) {
    ...
    await execute(
      conn,
      "UPDATE jobs SET claude_session_id = ?, updated_at = ? WHERE id = ?",
      [proc.sessionId, new Date().toISOString(), jobId],
    );
  }
    ...
    if (
      ...
      if (job.status === "paused") {
        await updateJobStatus(
          ...
          "running",
          "paused",
          "Resuming with injected message",
        );
      }
