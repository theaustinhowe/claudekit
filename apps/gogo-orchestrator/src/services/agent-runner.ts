...
export async function startJobRun(
  ...
  await execute(
    ...
    ["running", now, jobId, "queued"],
  );

  const claimed = await queryOne<DbJob>(
    conn,
  ...
  const revertToFailed = async (reason: string) => {
    ...
    const failedJob = await queryOne<DbJob>(
      conn,
  ...
  (async () => {
    try {
      ...
      await emitLog(
        jobId,
        "system",
        `Ensuring base repository clone at ${gitConfig.workdir}/.repo`,
        logState,
      );
      try {
      ...
      await emitLog(
        jobId,
        "system",
        "Fetching latest updates from origin...",
        logState,
      );
      try {
      ...
      await emitLog(
        jobId,
        "system",
        `Creating worktree for ${worktreeLabel}...`,
        logState,
      );
      try {
        ...
        await execute(
          conn,
          "UPDATE jobs SET branch = ?, worktree_path = ?, updated_at = ? WHERE id = ?",
          [branch, worktreePath, updateNow, jobId],
        );
        ...
        await emitLog(
          jobId,
          "system",
          `Workspace ready at ${worktreePath}`,
          logState,
        );
      } catch (error: unknown) {
        const err = error as Error;
      ...
      await emitLog(
        jobId,
        "system",
        "Workspace setup complete. Ready for agent execution.",
        logState,
      );
    } catch (error: unknown) {
      const err = error as Error;
