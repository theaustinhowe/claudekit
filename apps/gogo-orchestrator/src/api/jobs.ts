...
import { isRunning as isJobRunning, startJobRun, stopJobRun } from "../services/agent-runner.js";
import {
  injectMessage,
...
import { isRunning as isMockRunning, startMockRun, stopMockRun } from "../services/mock-agent.js";
import { checkJobForResponseById, enterNeedsInfo } from "../services/needs-info.js";
...
export const jobsRouter: FastifyPluginAsync = async (fastify) => {
  ...
  fastify.post<{ Params: { id: string }; Body: unknown }>("/:id/actions", async (request, reply) => {
    ...
    if (action.type === "request_info") {
      if (job.status !== "running") {
        return reply.status(400).send({ error: "Can only request info from a running job" });
      }

      try {
        await enterNeedsInfo(request.params.id, action.payload.question);
        // Fetch the updated job to return
        const updatedRow = await queryOne<DbJob>(conn, "SELECT * FROM jobs WHERE id = ?", [request.params.id]);
        return { data: updatedRow ? mapJob(updatedRow) : null };
      } catch (error) {
        const message = error instanceof Error ? error.message : "Failed to post question to GitHub";
    ...
    if (action.type === "force_stop") {
      ...
      if (stopFailed) {
        const failResult = await applyActionAtomic(request.params.id, "cancel", {
        ...
        // Fetch the updated job to return
        const failedRow = await queryOne<DbJob>(conn, "SELECT * FROM jobs WHERE id = ?", [request.params.id]);
    ...
    if (action.type === "inject" && "payload" in action) {
      ...
      const injectResult = await injectMessage(job.id, action.payload.message, mode);
  ...
  fastify.post<{ Params: { id: string } }>("/:id/mock-run", async (request, reply) => {
    // Check if in development mode
    if (process.env.NODE_ENV === "production") {
      return reply.status(403).send({ error: "Mock runs are not available in production" });
    }
  ...
  fastify.post<{ Params: { id: string } }>("/:id/start-claude", async (request, reply) => {
  ...
  fastify.post<{ Params: { id: string }; Body: { message?: string } }>("/:id/resume-claude", async (request, reply) => {
  ...
  fastify.post<{ Params: { id: string }; Body: { agentType?: string } }>("/:id/start-agent", async (request, reply) => {
    const result = await startAgent(request.params.id, request.body?.agentType);
    if (!result.success) {
      return reply.status(400).send({ error: result.error });
    }
  ...
  fastify.post<{
    ...
    const result = await resumeAgent(request.params.id, request.body?.message, request.body?.agentType);
    if (!result.success) {
      return reply.status(400).send({ error: result.error });
    }
  ...
  fastify.post<{ Params: { id: string } }>("/:id/create-pr", async (request, reply) => {
  ...
  fastify.post<{ Params: { id: string } }>("/:id/check-response", async (request, reply) => {
    ...
    if (job.status !== "needs_info") {
      return reply.status(400).send({ error: "Job is not in needs_info state" });
    }
    ...
    try {
      ...
      return {
        ...
        message: result.responseFound ? "Response found, job resumed" : "No new response found",
      };
    } catch (error) {
