...
export const worktreesRouter: FastifyPluginAsync = async (fastify) => {
  ...
  fastify.get("/", async (_request, _reply) => {
    ...
    const activeRepoRows = await queryAll<DbRepository>(
      conn,
    ...
    const jobRows = await queryAll<DbJob>(
      conn,
      `SELECT * FROM jobs WHERE ${inClause}`,
      inParams,
    );
      ...
      if (validation.valid) {
        ...
        if (workspaceSettings) {
          try {
            ...
            fastify.log.debug(
              { error },
              "Failed to list worktrees for legacy workspace settings",
            );
          }
        }
      }
  ...
  fastify.get<{ Params: { jobId: string } }>(
    ...
    async (request, reply) => {
      ...
      if (!job.worktreePath) {
        return reply
          .status(400)
          .send({ error: "Job has no worktree", files: [] });
      }
      ...
      if (job.repositoryId) {
        ...
        if (!validation.valid) {
          return reply
            .status(400)
            .send({ error: "Workspace settings invalid" });
        }
        ...
        if (!workspaceSettings) {
          return reply
            .status(500)
            .send({ error: "Failed to load workspace settings" });
        }
  ...
  fastify.get<{ Querystring: { worktreePath: string } }>(
    "/by-path/changes",
    async (request, reply) => {
  ...
  fastify.get<{ Querystring: { worktreePath: string; path: string } }>(
    "/by-path/diff",
    async (request, reply) => {
        ...
        try {
          ...
          if (!pr.merged) {
            return reply.status(400).send({
              error: "Cannot cleanup job",
              details:
                "PR exists but is not merged. Merge or close the PR first.",
            });
          }
        } catch (error: unknown) {
          const err = error as Error;
  ...
  fastify.post<{ Body: CleanupRequestBody }>(
    "/cleanup",
    async (request, reply) => {
      ...
      const statusesToClean = includeStatuses
        ? includeStatuses.filter((s) => allowedStatuses.includes(s))
        : ["done"]; // Default to only "done"
      ...
      const { clause: inClause, params: inParams } = buildInClause(
        "status",
        statusesToClean,
      );
      let jobsToClean = (
        await queryAll<DbJob>(
          conn,
          `SELECT * FROM jobs WHERE ${inClause}`,
          inParams,
        )
      ...
      const repoIds = Array.from(jobsByRepo.keys()).filter(
        (id): id is string => id !== null,
      );
      if (repoIds.length > 0) {
        const { clause: repoInClause, params: repoInParams } = buildInClause(
          "id",
          repoIds,
        );
      ...
      for (const job of jobsToClean) {
        ...
        if (!gitConfig) {
          result.skipped.push({
            jobId: job.id,
            reason: job.repositoryId
              ? "Repository config not found"
              : "Legacy workspace settings not configured",
          });
          continue;
        }
