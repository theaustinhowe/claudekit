...
function buildRequest(appId: string) {
  const req = new Request(`http://localhost:2000/api/apps/${appId}/restart`, { method: "POST" });
...
describe("POST /api/apps/[id]/restart", () => {
  it("sends POST to daemon control endpoint with app id", async () => {
    ...
    expect(fetchMock).toHaveBeenCalledWith("http://localhost:2999/restart/gadget", {
      method: "POST",
    });
  });
  ...
  it("returns daemon response data and status", async () => {
    fetchMock.mockResolvedValue({
      status: 200,
      json: () => Promise.resolve({ ok: true, pid: 12345 }),
    });
    ...
    expect(data).toEqual({ ok: true, pid: 12345 });
  });
  ...
  it("encodes app id in URL to handle special characters", async () => {
    ...
    expect(fetchMock).toHaveBeenCalledWith("http://localhost:2999/restart/gogo-orchestrator", {
      method: "POST",
    });
  });
