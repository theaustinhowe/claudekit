...
export function CodeFileViewer({ file, imageUrl, truncated, onShowFull, onOpenInFinder }: CodeFileViewerProps) {
      ...
      <div className="border rounded-lg @container">
        ...
        {resolvedImageUrl ? (
          <div className="flex items-center justify-center p-6 bg-[repeating-conic-gradient(var(--color-muted)_0%_25%,transparent_0%_50%)] bg-[length:16px_16px]">
            {/* biome-ignore lint/performance/noImgElement: dynamic repo image from API route, not optimizable by next/image */}
            <img
      ...
      <div className="flex items-center justify-between px-3 py-2 border-b bg-muted/50 gap-2">
        <TooltipProvider>
          <div className="flex items-center gap-1.5 min-w-0">
            ...
            {/* Narrow containers: info icon with popover */}
            <Popover>
              <Tooltip>
                <TooltipTrigger asChild>
                  <PopoverTrigger asChild>
            ...
            {/* Wide containers: inline metadata */}
            <Badge variant="outline" className="text-xs shrink-0 hidden @[600px]:inline-flex">
              {file.language}
            </Badge>
...
function PreviewContent({ file }: { file: CodeFileContent }) {
