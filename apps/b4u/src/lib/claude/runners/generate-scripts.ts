...
export function createGenerateScriptsRunner(runId: string): SessionRunner {
  return async ({ onProgress, signal }) => {
    ...
    const summaryRows = await queryAll<{
      ...
      framework: string;
      project_path: string;
    }>(conn, "SELECT name, framework, project_path FROM project_summary WHERE run_id = ? LIMIT 1", [runId]);
    ...
    try {
      const jsonStr = extractJsonObject(scriptsResult.stdout);
    ...
    try {
      const jsonStr2 = extractJsonObject(voiceoverResult.stdout);
      if (!jsonStr2) throw new Error("No JSON found in Claude output");
      voiceover = JSON.parse(jsonStr2);
    } catch (err) {
    ...
    // Save voiceover data to flow_voiceover and chapter markers
    try {
      await execute(conn, "BEGIN TRANSACTION");
