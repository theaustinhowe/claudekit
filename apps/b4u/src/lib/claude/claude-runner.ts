...
export function runClaude(options: RunClaudeOptions): Promise<ClaudeResult> {
  ...
  return new Promise((resolve, reject) => {
    ...
    // Spawn health check — warn if no output after 30s
    const SPAWN_TIMEOUT_MS = 30_000;
    const spawnHealthTimer = setTimeout(() => {
      if (bytesReceived === 0 && !settled) {
        onProgress({
          message: "Claude CLI has not responded after 30s — verify it's installed and in PATH",
          bytesReceived: 0,
          log: "[warn] No output from Claude CLI after 30s",
          logType: "status",
        });
      }
    }, SPAWN_TIMEOUT_MS);
    ...
    const processEvent = (evt: Evt): { log?: string; logType?: LogType; chunk?: string } => {
      ...
      const blocks = msg?.content as Evt[] | undefined;
      if (!blocks) return {};
